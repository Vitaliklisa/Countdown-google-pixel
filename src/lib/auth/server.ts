...
// The broker-backed `genericOAuth` plugin that used to live here is gone: Google
// is now offered DIRECTLY via `socialProviders.google` above, and the provider
// list is empty (see providers.ts), so a genericOAuth plugin with zero providers
// would register nothing. Removing it also removes the X/Twitter button.
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      enabled: true,
      // Google is the only social provider now; GATE_PROVIDER_ID covers the
      // gate-signed-in session used by the platform preview.
...
// `providers.ts` so the client can import it too. It is currently empty —
// Google is offered directly (see `socialProviders` above).
